...
function Card({ children, title, color, description, orientation = true }: Props) {
  return (
    <div className={`flex ${orientation ? "flex-col w-1/3" : "flex-row"} mx-2 text-black ${orientation ? "p-8 px-16 " : "p-2 items-center"}`} style={{ backgroundColor: color, borderRadius: "1.5rem",}}>
      {children}
